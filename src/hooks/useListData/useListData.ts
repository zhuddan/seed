...
// import { isEqual, sleep } from '@cloud/shared';
// import { onLoad } from '@dcloudio/uni-app';
type UniLoadMoreStatus = string;
export interface ListDataStatus {
  /**
   * 是否报错
   */
  error: boolean;
...
interface ListDataStatusListType<T extends AnyObject = AnyObject> extends ListDataStatus {
  list: T[];
  total: number;
}
...
export interface ListActions<T extends AnyObject = AnyObject> {
   ...
   * 分页刷新页面数据
   */
  readonly getHackData: () => Promise<void>;
}
...
export function useListData<T extends AnyObject = AnyObject>(
   ...
   * 底部状态 是否 显示
   */
  const isEmpty = computed(() => _Empty(list.value));

  const finished = ref(false);
  // if (flush == 'immediate') {
  //   getList();
  // }
  // else if (flush == 'mounted') {
  //   onMounted(() => {
  //     nextTick(() => {
  //       getList();
  //     });
  ...
  function updateStatus() {
    nextTick(() => {
      ...
      status.value = finished.value ? 'noMore' : 'more';
   ...
   * 请求数据
   */
  function getData(params: ListQuery<T> = paramsComputedRef.value) {
    const cbStatus = {
