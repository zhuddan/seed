...
export function generatedSvgIconType(isBuild: boolean) {
  if (isBuild) return;
  const iconsDir = path.resolve(__dirname, '../src/renderer/src/assets/icons');
...
declare type SvgIconType = ${iconTypes.map(e => `'${e}'`).join(' | ')};`;
    fs.writeFileSync(path.resolve(__dirname, '../src/renderer/types', 'svg-icon-type.d.ts'), str);
  }
