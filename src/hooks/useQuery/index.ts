import { isJSONString, logger, mergeDeep } from '@/utils/helpers';
 ...
 * @param defaultValue
 */
export function useQuery<T extends AnyObject = AnyObject>(defaultValue: T = {} as T, debugMsg = true) {
  // defaultValue 仅在 dev 环境下生效
  const query = ref<T>(__DEV__ ? defaultValue : {} as T);
  onLoad((_query) => {
    if (!_query) return;
    // if (__DEV__) {
    //   console.log('<query>');
...
function getJson(_query: AnyObject) {
  const data: AnyObject = {};
  for (const key in _query) {
    if (Object.prototype.hasOwnProperty.call(_query, key)) {
      const value = decodeURIComponent(decodeURIComponent(_query[key]));
